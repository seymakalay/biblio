...
M <- convert2df("savedrecs.174.txt", dbsource = "wos", format = "plaintext") 
...
colnames(mst.ctd.CO) <- c( "Country","Articles","SCP","MCP","%Freq","%MCP","%SCP","TGC", "AAC"  ) #AAC STANDS FOR AVERAGE ARTICLE CITATATION
#
rownames(mst.ctd.CO ) <- NULL
print(mst.ctd.CO )

mst.ctd.CO   <- xtable(mst.ctd.CO[c(1:20),],   digits =2, 
                    caption = 'Most Influancial Countries',label="tab:Most-Cited-Countries", scalebox='0.80')

print(mst.ctd.CO, file = 'mst.ctd.CO.tex',  caption.placement = 'top' ) #, size="\\fontsize{12pt}{11pt}\\selectfont")
...
mst.ctd.PA <- lclcttion.M.savedrecs.174[, c("AU", "Title", "Year","LCS","GCS", "Average.per.Year")]
...
gsub(";",";\\s",mst.ctd.PA$AU)
as.data.frame(apply(lclcttion.M.savedrecs.174,2,function(x)gsub('\\s+', '',x)))
...
names(mst.ctd.PA)
#mst.ctd.PA <- mst.ctd.PA[, c("Paper",#"Year", 
#                             "LCS","GCS","LCS/t","Average.per.Year")]   
                                       
mst.ctd.PA <- mst.ctd.PA[, c("AU", "Title", "Year", 
                             "LCS","GCS","LCS/t","Average.per.Year")]  

colnames(mst.ctd.PA) <- c("AU", "Title", "Year", 
                          "TLC","TGC","TLC/t", "TGC/t")  
...
mst.ctd.PA   <- xtable(mst.ctd.PA [c(1:20),],   digits = 2, 
                       caption = 'Most Influential Articles',label="tab:Most-Cited-Papers", scalebox='0.80')

#print(mst.ctd.PA, file = 'mst.ctd.PA.tex',  caption.placement = 'top') 
...
print(xtable(mst.ctd.PA,
             ...
             align=c(
               ...
               "p{0.7cm}|")),
      hline.after=c(-1, 0),
      file = 'mst.ctd.PA.tex' )#,tabular.environment = "longtable")#, hline.after=c(-1, 0), tabular.environment = "longtable")
...
mst.ctd.JOR <- lclcttion.M.savedrecs.174 %>%
  dplyr::group_by(J9)%>%
  dplyr::summarise(`LCS/t` = sum(`LCS/t`),
                    Affiliation.Paper = n(),
                    sum.LCS.t = sum(`LCS/t`),
...
mst.ctd.JOR   <- xtable(mst.ctd.JOR[c(1:20),],   digits =0, 
...
mst.ctd.AFF <- lclcttion.M.savedrecs.174  %>%
             ...
             dplyr::arrange(desc(sum.GCS)) #this comes  ""  2   28 cause there are 2 ; at the end AU_UN indicate no uni ; was an excedent

mst.ctd.AFF <- na.omit(mst.ctd.AFF)
...
mst.ctd.AFF   <- xtable(mst.ctd.AFF[c(1:20),],   digits = 0, 
                        caption = 'Most Citated Affliations',label="tab:Most-Cited-Affliations", scalebox='0.80')

print(mst.ctd.AFF, file = 'mst.ctd.AFF.tex',  caption.placement = 'top' ) 
...
my.table8 <- lclcttion.M.savedrecs.174[, c( "Paper","Title",#"Authors",#"AU_UN",
                                            "LCS","GCS","GCS/t","LCS/t","Average Article Citations")] #AAC=Average Article Citations
colnames(my.table8) <- c( "Paper","Title",#"Authors",#"AU_UN",
                          "LCS","GCS","GCS/t","LCS/t","AAC")

my.table8$LCS <-  as.integer(my.table8$LCS)
my.table8$GCS <-  as.integer(my.table8$GCS)
my.table8 $`GCS/t` <- as.integer(my.table8 $`GCS/t`)
my.table8$`LCS/t` <- as.integer(my.table8$`LCS/t`)
my.table8$ACC <- as.integer(my.table8$ACC)

my.table8 <-  my.table8 [order(-my.table8$`GCS/t`),]
head(my.table8)
write.csv(my.table8, file = "my.table8.csv")
x.big <- xtable(my.table8,  caption = 'Selected papers for bibliometric anaysis',label="tab:Biblio", scalebox='0.80')
...
print(xtable(x.big,
             ...
             align=c(
               ...
               "p{0.7cm}|")),
       hline.after=c(-1, 0),
      file = 'x.big.tex' ,tabular.environment = "longtable")
...
#print(x.big, hline.after=c(-1, 0), tabular.environment = "longtable")
#print(x.big, hline.after=c(-1, 0), tabular.environment = "longtable", file = 'x.big.tex',  caption.placement = 'top' ) 
